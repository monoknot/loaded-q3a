...
/* LQ3A: CG_DrawStringExt2() is essentially CG_DrawStringExt() with an additional uFlags parameter
	which provides us with additional functionality. CG_DrawStringExt() just calls CG_DrawStringExt2()
	with uFlags set to UI_LEFT to preserve the original implementation without the need to refactor large
	amounts of code. */
ID_INLINE void CG_DrawStringExt(int x, int y, const char *string, const float *setColor, 
		qboolean forceColor, qboolean shadow, int charWidth, int charHeight, int maxChars)
{
	CG_DrawStringExt2(x, y, string, setColor, forceColor, shadow, charWidth, charHeight, maxChars, UI_LEFT);
}
