...
void CG_ParseServerinfo( void ) {
	...
	Com_sprintf(cgs.mapname, sizeof(cgs.mapname), "maps/%s.bsp", cgs.cMap);
...
void CG_SetConfigValues( void ) {
	...
	/* LQ3A */
	LQ3A_ParseLoadedConfigString(CG_ConfigString(CS_LQ3A_CONFIG), MAX_STRING_CHARS);
	LQ3A_ParseMotd(CG_ConfigString(CS_MOTD), qfalse);
	LQ3A_ParseHighScores(CG_ConfigString(CS_LQ3A_HIGHSCORES), MAX_STRING_CHARS);
	LQ3A_ParseLimitsConfigString(CG_ConfigString(CS_LQ3A_LIMITS), MAX_STRING_CHARS);
}
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
		...
		/* LQ3A: Record the time the intermission started.
			We use this to determine when to display the high score table. */
		if (cg.intermissionStarted)
		{
			cg.iIntermissionStartTime = cg.time;
		}
	...
	{
		LQ3A_ParseItemsConfigString(str);
	}
	else if (num == CS_LQ3A_CONFIG)
	{
		LQ3A_ParseLoadedConfigString(str, strlen(str));
	}
	else if (num == CS_MOTD)
	{
		LQ3A_ParseMotd(str, qtrue);
	}
	else if (num == CS_LQ3A_HIGHSCORES)
	{
		LQ3A_ParseHighScores(str, strlen(str));
	}
	else if (num == CS_LQ3A_LIMITS)
	{
		LQ3A_ParseLimitsConfigString(str, strlen(str));
	}
}
...
/* LQ3A */
	...
	UNREFERENCED_PARAMETER(voiceOnly);
	UNREFERENCED_PARAMETER(clientNum);
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "cp" ) ) {
		/* LQ3A */
		LQ3A_ParseCenterPrint(CG_Argv(1), SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH);
		return;
	}
