...
typedef struct {
	...
	int				ping;
	int				time;
	int				scoreFlags; /* LQ3A: Used to transmit SPECTATOR_FOLLOW and SPECTATOR_FOLLOW_THIRDPERSON target client number + 1. */
	int				powerUps;
	int				accuracy;
	int				impressiveCount;
	int				excellentCount;
...
typedef struct {
	...
	/* LQ3A: Changed from qboolean to int.
		A non-zero value indicates the intermission is active, the value represents the length in milliseconds. */
	int			intermissionStarted;	// don't play voice rewards, because game will end shortly

	/* LQ3A: cg.time the intermission was started. */
	int			iIntermissionStartTime;
	...
	int				iNextLayoutEndTime;

	/* LQ3A: Layout functionality renders these obsolite. */
/*
	qboolean	showScores;
...
*/
	...
	// centerprinting
	/* LQ3A: Obsolete, replaced by the LQ3A layout system. */
//	int			centerPrintTime;
...
typedef struct {
	...
	qboolean			bMissionPack;					/**< True when the mission pack content is present on the server. */
	lq3a_motd_t			Motd;							/**< Message of the day data. */
	lq3a_highscores_t	HighScores;						/**< Highscore data. */
	uint				uLatchedLayoutKeys;				/**< Bitmask which described the current latched layout keys. */
	int					iAmmoLimits[WP_NUM_WEAPONS];	/**< Array containing ammo limits parsed in from the
																CS_LQ3A_LIMITS configuration string. */
	int					iShellSpread;					/**< Matches the server side g_damageShellSpread cvar.
																Parsed in from the CS_LQ3A_CONFIG configuration string. */
	qboolean  newHud;
...
/* LQ3A */
extern vmCvar_t		cg_spectatorPassword;
extern vmCvar_t		cg_hookColor;

//
// cg_main.c
//
/* LQ3A: Removed 'const' declaration. */
char *CG_ConfigString( int index );
