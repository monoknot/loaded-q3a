...
#define SPACING  5
	...
	/* LQ3A: Use the rail style communicated to us by the server, while this normally corrisponds to the cg_oldRail cvar
		it allows the style to be forced server side.
		The old rail style has also been changed to a spiral only which is a true match to the old style. */
	...
	/* LQ3A: Loop until we reach the destination. */
    for (i = 0; 1; i += SPACING) {
		if (i != skip)
		{
			skip = i + SPACING;
			...
			/* LQ3A: Increased the speed at which the sprite moves away from center. */
            le->pos.trDelta[0] = axis[j][0]*10;
            le->pos.trDelta[1] = axis[j][1]*10;
            le->pos.trDelta[2] = axis[j][2]*10;

			/* LQ3A: Break when we reach the end point. */
			VectorSubtract(le->pos.trBase, start, pos);

			if (VectorNormalize(pos) >= len)
			{
				break;
			}
		}
...
void CG_GrappleTrail( centity_t *ent, const weaponInfo_t *wi ) {
	...
	/* LQ3A */
	pClientInfo = &cgs.clientinfo[es->otherEntityNum];
	...
	/* LQ3A */
	beam.customShader	= cgs.media.hookShader;

	AxisClear( beam.axis );

	/* LQ3A */
	beam.shaderRGBA[0]	= 255 * pClientInfo->vHookColor[0];
	beam.shaderRGBA[1]	= 255 * pClientInfo->vHookColor[1];
	beam.shaderRGBA[2]	= 255 * pClientInfo->vHookColor[2];
...
void CG_RegisterWeapon( int weaponNum ) {
	...
	case WP_GRAPPLING_HOOK:
		...
		/* LQ3A: Register the media borrowed form the lightning gun as well. */
		cgs.media.lightningExplosionModel = trap_R_RegisterModel("models/weaphits/crackle.md3");
		...
		/* LQ3A */
		cgs.media.hookShader = trap_R_RegisterShader("misc/lq3a_hook");
		break;
...
static void CG_AddWeaponWithPowerups( refEntity_t *gun, int powerups ) {
	...
	if ( powerups & ( 1 << PW_INVIS ) ) {
		...
		{
			/* Flash the regeneration effect around the gun. */
			if (((cg.time / 100) % 10) == 1)
			{
		...
		{
			/* Draw the protection shield around the gun. */
...
void CG_MissileHitWall( int weapon, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType ) {
	...
	/* LQ3A */
	UNREFERENCED_PARAMETER(soundType);
...
static void CG_ShotgunPattern( vec3_t origin, vec3_t origin2, int seed, int otherEntNum ) {
	...
	for ( i = 0 ; i < DEFAULT_SHOTGUN_COUNT ; i++ ) {
		...
		/* LQ3A */
		r = Q_crandom( &seed ) * cgs.iShellSpread * 16;
		u = Q_crandom( &seed ) * cgs.iShellSpread * 16;
