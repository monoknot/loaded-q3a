...
void CG_NewClientInfo( int clientNum ) {
	...
	/* LQ3A */
	v = Info_ValueForKey(configstring, "or");
	newInfo.bOldRailStyle = atoi(v) ? qtrue : qfalse;

	/* LQ3A */
	v = Info_ValueForKey(configstring, "hkc");
	CG_ColorFromString(v, newInfo.vHookColor);
...
static void CG_PlayerPowerups( centity_t *cent, refEntity_t *torso ) {
	...
	/* LQ3A: The protection sheld emits an ambiant glow. */
	if (powerups & (1 << PW_PROTECTION))
	{
		trap_R_AddLightToScene(cent->lerpOrigin, 180 + (rand() & 31), 0.2f, 1.0f, 0.2f);
	}
...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	/* LQ3A: Since the camping state results in death we should
		ensure it takes preference over all other sprites. */
	if (cent->currentState.eFlags & EF_CAMPING)
	{
		CG_PlayerFloatSprite(cent, cgs.media.camping);
		return;
	}
...
void CG_AddRefEntityWithPowerups( refEntity_t *ent, entityState_t *state, int team ) {
	...
	if ( state->powerups & ( 1 << PW_INVIS ) ) {
		...
		/* LQ3A: Draw the protection shield around the entities that have it. */
