...
static float CG_DrawScores( float y ) {
	...
	if ( cgs.gametype >= GT_TEAM ) {
		...
		/* LQ3A: Draw the protection shield timer when active. */
		if ((cg.snap->ps.powerups[PW_PROTECTION]) && !(cg.renderingThirdPerson))
		{
			s1 = ((cg.snap->ps.powerups[PW_PROTECTION] - cg.time) / 1000) + 1;

			/* Cap display value to 99. */
			if (s1 > 99)
				s1 = 99;
...
void CG_CenterPrint( const char *str, int y, int charWidth ) {
	...
	/* LQ3A: Function rewritten. */
	LQ3A_ParseCenterPrint(str, y, charWidth);
}
...
===================
*/
/* LQ3A: Function removed, replaced with LQ3A_DrawCenterString(). */
/*
...
static void CG_DrawSpectator(void) {
	...
	vec4_t	cColor = {1.0f, 1.0f, 1.0f, 1.0f};
	char	cMessage[MAX_STRING_CHARS];
	...
	{
		Com_sprintf(cMessage, sizeof(cMessage), "Press %sESC%s and use the %sJOIN%s menu to play",
			S_COLOR_BLUE, S_COLOR_WHITE, S_COLOR_BLUE, S_COLOR_WHITE);
	...
	{
		Com_sprintf(cMessage, sizeof(cMessage), "Press %sFIRE%s to join or %sUSE%s to follow",
			S_COLOR_BLUE, S_COLOR_WHITE, S_COLOR_BLUE, S_COLOR_WHITE);

		CG_DrawStringExt2(320, 446, cMessage, cColor, qfalse, qtrue, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, 0, UI_CENTER);
	}
}
...
static qboolean CG_DrawScoreboard() {
	...
	/* LQ3A */
	if ( cg.warmup && (cg.iLayout == LQ3A_LAYOUT_NONE)) {
		return qfalse;
	}

	/* LQ3A */
	if ((cg.iLayout == LQ3A_LAYOUT_SCOREBOARD) || cg.predictedPlayerState.pm_type == PM_DEAD || cg.predictedPlayerState.pm_type == PM_INTERMISSION ) {
		fade = 1.0;
		fadeColor = colorWhite;
	} else {
		/* LQ3A */
		fadeColor = CG_FadeColor( cg.iLayoutEndTime, FADE_TIME );
		if ( !fadeColor ) {
...
//		CG_DrawCrosshair();
	...
	} else if (cg.predictedPlayerState.pm_flags & PMF_FOLLOW)
	{
		...
		if (cg.snap->ps.stats[STAT_HEALTH] > 0) {
			...
			else if ((cg.iLayout == LQ3A_LAYOUT_MOTD) || (cg.iLayout == LQ3A_LAYOUT_CENTERSTRING))
			{
				CG_DrawCrosshair();
				CG_DrawCrosshairNames();
			}
