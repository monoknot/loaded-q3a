...
void DeathmatchScoreboardMessage( gentity_t *ent ) {
	...
	for (i=0 ; i < numSorted ; i++) {
		...
		/* LQ3A */
		pScoreEntity = g_entities[level.sortedClients[i]];
		...
		/* LQ3A: Use scoreFlags to transmit the target client number.
			We have to ignore bots since they're always trying to enter the game when spectating and
			subsequently start following clients when all the g_maxGameClients has been reached. */
		if ((!(pScoreEntity.r.svFlags & SVF_BOT)) &&
				((cl->sess.spectatorState == SPECTATOR_FOLLOW) || (cl->sess.spectatorState == SPECTATOR_FOLLOW_THIRDPERSON)))
		{
			if (LQ3A_GetClientIndexInSortedScores(cl->sess.spectatorClient, &scoreFlags))
			{
				/* Index + 1 = Position. */
				scoreFlags++;
			}
...
{
	/* LQ3A: Changed broadcasts from center print to print. */
...
void StopFollowing( gentity_t *ent ) {
	...
	/* LQ3A: Set the clients view angles to match thoughs of our old follow target.
		This prevents the camera from jumping to the old out-of-date view angles. */
	gclient_t	*pClient;
	int			iClient;
	...
	{
		iClient = ent->client->sess.spectatorClient;

		if (iClient == -1)
			iClient = level.follow1;
		else if (iClient == -2)
			iClient = level.follow2;

		if (iClient >= 0)
		{
			pClient = &level.clients[iClient];

			/** @todo: Adjust the view angles to match the third person offset. */
			if (ent->client->sess.spectatorState == SPECTATOR_FOLLOW_THIRDPERSON)
			{
			}

			SetClientViewAngle(ent, pClient->ps.viewangles);
		}
	}
	...
	/* LQ3A: Leave intact at the intermission to ensure the final scoreboard displays our follow target. */
...
void Cmd_Team_f( gentity_t *ent ) {
	...
	/* LQ3A */
	if ((trap_Argc() != 2) && (*g_spectatorPassword.string == 0))
	{
...
void Cmd_FollowCycle_f( gentity_t *ent, int dir, lq3a_follow_type_t eType) {
	...
	{
		trap_SendServerCommand(ent - g_entities, "cp \"There are no players\navailable to follow\"");
		return;
	}
...
//		case LQ3A_FOLLOW_AUTO:
		default:
			...
			/* If spectatorState is not valid, default to SPECTATOR_FOLLOW_THIRDPERSON. */
			if ((ent->client->sess.spectatorState != SPECTATOR_FOLLOW) &&
					(ent->client->sess.spectatorState != SPECTATOR_FOLLOW_THIRDPERSON))
			{
				ent->client->sess.spectatorState = SPECTATOR_FOLLOW_THIRDPERSON;
	...
	/* LQ3A */
	trap_SendConsoleCommand(ent - g_entities, "cp \"Unable to select a player to follow.\n\"");
}
...
void ClientCommand( int clientNum ) {
	...
	if (Q_stricmp (cmd, "give") == 0)
		...
		/* LQ3A */
		Cmd_Follow_f(ent, LQ3A_FOLLOW_TYPE_THIRDPERSON);
	else if (Q_stricmp (cmd, "follownext") == 0)
		/* LQ3A */
		Cmd_FollowCycle_f(ent, 1, LQ3A_FOLLOW_AUTO);
