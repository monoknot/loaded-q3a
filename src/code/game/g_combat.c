...
void TossClientItems( gentity_t *self ) {
	...
	/* LQ3A: Added typecast to weapon_t. */
	...
	/* LQ3A: Do not drop the current weapon if its part of the start inventory. */
	if (g_dropItemsOnDeath.integer && !(level.StartItems.iWeapons & (1 << weapon)) && self->client->ps.ammo[weapon])
	{
	...
	if ( g_gametype.integer != GT_TEAM ) {
		...
		for ( i = 1 ; i < PW_NUM_POWERUPS ; i++ ) {
			if ( self->client->ps.powerups[ i ] > level.time ) {
				...
				/* LQ3A: Always drop CTF flags. */
				if (!g_dropItemsOnDeath.integer &&
...
void body_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	/* LQ3A */
	UNREFERENCED_PARAMETER(inflictor);
	UNREFERENCED_PARAMETER(attacker);
	UNREFERENCED_PARAMETER(damage);
	UNREFERENCED_PARAMETER(meansOfDeath);
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	/* LQ3A: Camping also returns the flags. */
	if ((meansOfDeath == MOD_SUICIDE) || (meansOfDeath == MOD_CAMPING)) {
	...
	/* LQ3A: Remove the camping sprite. */
	self->s.eFlags &= ~EF_CAMPING;
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	/* LQ3A: Kickback is reduced when the protection shell is active.
		This prevents knocking freshly spawned players into the void. */
	...
	if ( !(dflags & DAMAGE_NO_PROTECTION) ) {
		...
		/* LQ3A: The protection shield prevents any damage. */
		if (client && client->ps.powerups[PW_PROTECTION])
		{
			return;
		}
	...
	/* LQ3A */
	if ((targ == attacker) && (g_damageSelfFactor.value > 0.0f))
	{
		damage *= g_damageSelfFactor.value;
	}
