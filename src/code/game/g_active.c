...
void P_WorldEffects( gentity_t *ent ) {
	...
	if ( waterlevel == 3 ) {
		...
		/* LQ3A */
		ent->client->airOutTime = level.time + (g_waterTime.value * 1000);
	...
	if (waterlevel && 
		...
		if (ent->health > 0
			...
			if ( envirosuit ) {
				...
				/* LQ3A */
				if ((ent->watertype & CONTENTS_LAVA) && (g_damageLava.integer > 0))
				{
					G_Damage(ent, NULL, NULL, NULL, NULL, g_damageLava.integer * waterlevel, 0, MOD_LAVA);
				}

				/* LQ3A */
				if ((ent->watertype & CONTENTS_SLIME) && (g_damageSlime.integer > 0))
				{
					G_Damage(ent, NULL, NULL, NULL, NULL, g_damageSlime.integer * waterlevel, 0, MOD_SLIME);
				}
			}
		}
	}
...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	if ((client->sess.spectatorState != SPECTATOR_FOLLOW) &&
			(client->sess.spectatorState != SPECTATOR_FOLLOW_THIRDPERSON))
	{
		/* LQ3A: Free floating spectators can fly through walls. */
	...
	/* LQ3A: Save the upmove value so we can detect changes. */
	client->oldUpMove	= client->upmove;
	client->upmove		= ucmd->upmove;

	/* LQ3A: 'Attack' either join's or cycles through
		clients we can follow depending on the spectator type. */
	if ((client->buttons & BUTTON_ATTACK) && !(client->oldbuttons & BUTTON_ATTACK))
	{
		...
		{
			trap_SendServerCommand(client - level.clients, "cp \"Unable to join game\nno vacant player slots available\"");
		}
	}

	/* LQ3A: 'Use' toggles between free and follow mode. */
	if ((client->buttons & BUTTON_USE_HOLDABLE) && !(client->oldbuttons & BUTTON_USE_HOLDABLE))
	{
		if (client->sess.spectatorState == SPECTATOR_FREE)
		{
			Cmd_FollowCycle_f(ent, 1, LQ3A_FOLLOW_TYPE_THIRDPERSON);
		}
		else if (client->sess.spectatorState == SPECTATOR_FOLLOW_THIRDPERSON)
		{
			StopFollowing(ent);
		}
	}

	/* LQ3A: 'moveup' and 'movedown' cycles through the clients in follow mode. */
	if (client->sess.spectatorState == SPECTATOR_FOLLOW_THIRDPERSON)
	{
		if (ucmd->upmove > 0)
		{
			if (client->oldUpMove <= 0)
			{
				/* Follow next client and preserve the follow type. */
		...
		else if (ucmd->upmove < 0)
		{
			if (client->oldUpMove >= 0)
			{
				/* Follow previous client and preserve the follow type. */
				Cmd_FollowCycle_f(ent, -1, LQ3A_FOLLOW_AUTO);
			}
		}
	}
...
//	qboolean	fired;
	...
	for ( i = oldEventSequence ; i < client->ps.eventSequence ; i++ ) {
		...
		case EV_FALL_FAR:
			...
			/* LQ3A */
			if ((g_dmflags.integer & DF_NO_FALLING) || (g_damageFallFactor.value <= 0.0f))
			{
				break;
			}
			...
			/* LQ3A */
			damage *= g_damageFallFactor.value;
...
//		G_Printf("serverTime >>>>>\n" );
	...
	/* LQ3A */
	LQ3A_ClientCampCheck(client);

	/* LQ3A */
	if (g_hookOffhand.integer)
	{
		if ((ucmd->buttons & BUTTON_HOOK))
		{
			if (!client->hook)
			{
				Weapon_GrapplingHook_Fire(ent);
			}
			/* Release the hook if its been attached for too long. */
			else if ((client->iHookTime >= 0) && (level.time > client->iHookTime))
			{
				Weapon_HookFree(client->hook);
			}
		}
